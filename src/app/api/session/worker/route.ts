...
async function addEvent(
    ...
    const { error } = await supabase
        .from('debate_events')
        .insert({ run_id, event_type, model, payload });
    if (error) console.error(`[addEvent] ${event_type} error:`, error.message);
}
...
async function callOpenRouter(
    ...
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
        ...
        body: JSON.stringify({ model, messages, provider: providerConfig }),
    });
...
export async function POST(req: Request) {
    ...
    try {
        ...
        // ──────────────────────────────────────────────
        console.log(`[Worker] Starting Round 1 for ${personas.length} personas...`);
            ...
            personas.map(async (p) => {
                ...
                try {
                    ...
                    const messages = [
                        {
                            role: 'system',
                            content: `You are the ${p.name}. Role: ${p.role}. Provide a concise, professional analysis.`,
                        },
                        { role: 'user', content: ideaRedacted },
                    ];

                    const out = await callOpenRouter(modelKey, messages, {
                        zdr: config.judge.zdr,
                    });
                    const text =
                        out?.choices?.[0]?.message?.content ||
                        `Analysis complete by ${p.name}.`;
                    ...
                    console.error(`[Worker] Round 1 error for ${p.id}:`, err);
                    await addEvent(supabase, runId, 'error', p.id, {
                        msg: `Failed Round 1: ${err.message}`,
                    });
        ...
        // ──────────────────────────────────────────────
        const transcriptR1 = round1Results
            .map((r) => `[${r.name}]: ${r.text}`)
            .join('\n\n');

        await addEvent(supabase, runId, 'system', null, {
            msg: 'Phase 2: Council Rebuttal & Cross-Examination',
        });
        console.log('[Worker] Starting Round 2 (Rebuttal)...');
        ...
        const round2Results = await Promise.all(
            personas.map(async (p) => {
                ...
                try {
                    ...
                    console.error(`[Worker] Round 2 error for ${p.id}:`, err);
                    return { id: p.id, name: p.name, text: '[Error in Round 2]' };
                }
            })
        );

        // ── Emit consensus after Round 2 ──
        await addEvent(supabase, runId, 'consensus', null, {
            coreSync: 72,
            global: 60,
            phase: 'after_round_2',
        ...
        // ──────────────────────────────────────────────
        const transcriptR2 = round2Results
            .map((r) => `[${r.name}]: ${r.text}`)
            .join('\n\n');
        ...
        const judgeMessages = [
            {
                role: 'system',
                content:
                    'You are the Judge. Consolidate the two rounds of debate into a final verdict and consensus score (0-100). Format: include "XX/100" somewhere in your response.',
            },
            {
                role: 'user',
                content: `Analyze the full debate:\n\n${fullTranscript}`,
            },
        ];
        ...
        try {
            ...
            const judgeText =
                judgeOut?.choices?.[0]?.message?.content ||
                'Judge finalized evaluation.';

            const scoreMatch = judgeText.match(/(\d{1,3})\/100/);
            finalScore = scoreMatch ? parseInt(scoreMatch[1]) : 85;

            await addEvent(supabase, runId, 'judge_note', 'judge', {
                text: judgeText,
                type: 'final_consensus',
                consensusDelta: finalScore,
            });
            ...
            console.error('[Worker] Judge Error:', err);
            await addEvent(supabase, runId, 'error', 'judge', {
                msg: `Judge failed: ${err.message}`,
            });
            ...
            await supabase
                ...
                .update({
                    ...
                    consensus_score: 0,
                    full_result: {
                        judge: `Judge failed: ${err.message}`,
                        round1: round1Results,
                        round2: round2Results,
                    },
                })
        ...
        // ──────────────────────────────────────────────
        // EMIT COMPLETE EVENT (critical for frontend redirect)
        // ──────────────────────────────────────────────
        ...
        console.log(`[Worker] ✅ Debate ${runId} complete. Score: ${finalScore}`);
